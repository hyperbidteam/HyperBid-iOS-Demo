...
- (void)loadAdButtonClickAction {
    [self showLog:[NSString stringWithFormat:@"%@",kLocalizeStr(@"LoadAdClicked")]];
    
    // Load ad - required step
    if (!self.appOpenAd) {
        self.appOpenAd = [[MCAppOpenAd alloc] initWithPlacementId:Hyperbid_iOS_splash containerView:[self footLogoView]];
    }
...
- (void)showAdButtonClickAction {
    ...
    if (!isReady) {
        [self showLog:[NSString stringWithFormat:@"%@",kLocalizeStr(@"AdNotReady")]];
